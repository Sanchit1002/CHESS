...
export const BotGame: React.FC<BotGameProps> = ({ boardTheme, color, onBack }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-amber-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 transition-colors duration-300">
      {/* Main Layout: Controls, Board, Move History */}
      <div className="flex flex-col md:flex-row items-start justify-center gap-6 w-full max-w-6xl mx-auto">
        ...
        {/* Chess Board and Suggestions - center everything */}
        <div className="flex flex-col items-center w-[480px] mx-auto">
          {/* Centered Offer Draw/Resign Buttons */}
          ...
          {showSuggestion && isMyTurn && suggestedMove === '...' && (
            <div className="mt-2 text-center text-amber-600 dark:text-amber-300 animate-pulse w-full">Calculating suggestion...</div>
          )}
          {isBotThinking && (
            <div className="mt-4 text-center text-amber-600 dark:text-amber-300 animate-pulse w-full">Bot is thinking...</div>
          )}
          {chess.isGameOver() && !showGameOverModal && (
            <div className="mt-4 text-center text-red-600 dark:text-red-400 font-bold w-full">Game Over: {chess.isCheckmate() ? (isMyTurn ? 'Bot wins!' : 'You win!') : 'Draw'}</div>
          )}
